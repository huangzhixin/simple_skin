...
def calcFrequenceClassFeatures(settingName):
    ...
    activity_start_frame = np.load(settings['Name_of_run'] + '/highFrequence_activity_start_frame.npy')
    activity_end_frame = np.load(settings['Name_of_run'] + '/highFrequence_activity_end_frame.npy')

    label = np.load(settings['Name_of_run'] + '/highFrequence_label.npy')

    frameFeatures = pd.read_pickle(settings['Name_of_run'] + '/frameFeatures.pkl')
    segmentFeatures = pd.read_pickle(settings['Name_of_run'] + '/highFrequence_segmentFeatures.pkl')

    frontsideCleanData = np.load(settings['Name_of_run'] + '/' + 'thresholdCleanData' + '.npy')
    ...
    top_areaFeature = np.zeros((number_of_segments))
    ...
    for i in range (0, number_of_segments):
        ...
        for i in range(0,number_of_segments):
            ...
            for j in range(activity_start_frame[i+1],activity_end_frame[i+1]):
                threshold = np.mean(frontsideCleanData[:, :, j])
                threshold = threshold * 2
                top_areaFeature[i]=top_areaFeature[i]+np.sum(frontsideCleanData[:,:,j]>threshold)
            top_areaFeature[i] = top_areaFeature[i]/(activity_end_frame[i+1]-activity_start_frame[i+1])
            #print top_areaFeature[i], u''+label[i+1]
        '''
        ...
        peakcount[i]=peaks[0].__len__()*100/(activity_end_frame[i+1]-activity_start_frame[i+1])
        vallycount[i]=peaks[1].__len__()*100/(activity_end_frame[i+1]-activity_start_frame[i+1])
    ...
    features = {
                #'top_areaFeature_segmented' : top_areaFeature,
        ...
        "changeOfFrontXWeightFeature": segmentFeatures["changeOfFrontXWeightFeature"],
        "changeOfFrontYWeightFeature": segmentFeatures["changeOfFrontYWeightFeature"]
    ...
    featureTable.to_pickle(settings['Name_of_run'] + '/highFrequence_ClassFeatures.pkl')
...
def labelFrequenceClass(settingName,nameOfLabel,saveName):
    ...
    #en_highFrequences = ['Scratch','Scratch', 'Scratch', 'Scratch', 'throw_leg', 'Scratch', 'throwing', 'throwing']
    en_highFrequences = ['Stab','Scratch', 'Draw', 'Touch', 'throw_leg', 'Dig', 'throwing', 'throwing_away']

    frequenceLabel = label.copy()
    for i in range(0,frequenceLabel.shape[0]):
        for j in range(0,highFrequences.__len__()):
            if label[i]==highFrequences[j]:
                frequenceLabel[i]=en_highFrequences[j]
                break
...
labelFrequenceClass('settings1','highFrequence_label','en_highFrequence_label')
calcFrequenceClassFeatures('settings2')
labelFrequenceClass('settings2','highFrequence_label','en_highFrequence_label')
calcFrequenceClassFeatures('settings3')
labelFrequenceClass('settings3','highFrequence_label','en_highFrequence_label')
calcFrequenceClassFeatures('settings6')
labelFrequenceClass('settings6','highFrequence_label','en_highFrequence_label')