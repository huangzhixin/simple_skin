...
class Classifier():
    ...
    def read_data(self, feature_table_name='segmentFeatures'):
        self.data_x = None
        self.data_y = None
        for i in range(0, self.settings.values.shape[0]):
            features_table = pd.read_pickle(self.settings.index[i] + '/' + feature_table_name + '.pkl')
            if i == 0:
                self.data_x = features_table.values[:, :-1]
                self.data_y = features_table.values[:, -1]
            else:
                new_data_x = features_table.values[:, :-1]
                new_data_y = features_table.values[:, -1]
    ...
    def auto_validation(self):
        ...
        for classifier in test_classifiers:
            ...
            model, accuracy, confusion_mat = model(self.data_x, self.data_y, 6)
